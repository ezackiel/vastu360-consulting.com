...
// Writes AI-detected rooms straight into the booking's q_<roomId> /
// q_<roomId>_width / q_<roomId>_length fields — the same fields the manual
// DirectionQuestions form produces, so every downstream calculation
// (scoring, Ayadi, Dosha, PDF) picks them up unchanged. Used by the
// auto-apply-on-upload flow below (no customer confirmation step).
function applyRoomsToBooking(booking, rooms) {
  let applied = 0;
  (rooms || []).forEach(r => {
...
// multiple pages/files. On upload, the file is immediately sent to
// floorPlanVision.analyzeFloorPlan and every detected room is applied
// straight into the booking (fully automatic — no separate customer
// confirmation step). If AI analysis fails (e.g. ANTHROPIC_API_KEY not
// configured, or the plan can't be read), the file still gets saved and
// queued for manual review by the Vastu360 team as a fallback.
app.post("/order/:orderId/floor-plan", auth.optionalAuth, (req, res) => {
  ...
  upload.single("floorPlan")(req, res, async (err) => {
    ...
    try {
      const fileBuffer = fs.readFileSync(path.join(UPLOAD_DIR, req.file.filename));
      const analysis = await analyzeFloorPlan(fileBuffer, req.file.mimetype);
      record.aiAnalysis = analysis;
      record.roomsApplied = applyRoomsToBooking(booking, analysis.rooms);
      record.autoApplied = true;
    } catch (analyzeErr) {
      analysisError = analyzeErr.message || "AI analysis failed.";
    ...
    notifyOwnerOfPurchase(
      ...
      req.params.orderId
    ); // reuse the existing notify hook so the team knows a plan came in
    ...
    res.status(201).json({
      file: {
        ...
        roomsApplied: record.roomsApplied
      },
      analysisError // null on success; a message string if AI analysis failed and it fell back to manual review
    });
  });
});
...
app.get("/order/:orderId/floor-plan", auth.optionalAuth, (req, res) => {
  ...
  const list = (floorPlans.get(req.params.orderId) || []).map(f => ({
    ...
    autoApplied: f.autoApplied || false, roomsApplied: f.roomsApplied || 0, analysisError: f.analysisError || null
  }));
  res.json({ files: list });
});
...
app.post("/order/:orderId/floor-plan/:fileId/apply", auth.optionalAuth, (req, res) => {
  ...
  const applied = applyRoomsToBooking(booking, rooms);
